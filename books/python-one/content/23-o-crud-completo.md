...
:::warning
Esse `create_all` no `main` é provisório e está aqui para você ver o projeto
rodando hoje. Ele tem dois problemas conhecidos: roda a cada partida, e não
altera tabela existente. O capítulo @cap:migrations remove essas três linhas
e coloca o Alembic no lugar — e essa é uma das poucas coisas deste livro que
você **não** deve levar para produção como está.
:::
