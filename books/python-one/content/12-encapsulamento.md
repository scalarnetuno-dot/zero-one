...
:::story O sétimo lugar
O Rafa achou o ajuste do estoque negativo. Levou dez minutos.

```python
produto.estoque = produto.estoque - 3
```

— Pronto. Conferi antes de atribuir. Não fica negativo.

A Bia abriu `baixar`, o único método onde a regra estava escrita. Ele não
tinha sido chamado.

— Você conferiu na sua cabeça. O produto não conferiu nada. Você escreveu o
número direto nele.

O Elias, de passagem, sem parar:

— Esse é o sétimo.
:::

Não existe, em Python, uma trava que o compilador recuse. Existe uma
convenção — um sublinhado na frente do nome — e ela é obedecida por
praticamente todo o ecossistema.

Parece frágil. O sétimo lugar existe justamente porque não havia nem a
placa: o número estava ali, público, e qualquer script de fim de tarde
podia escrevê-lo.
