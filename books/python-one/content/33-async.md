...
As rotas da cooperativa até aqui foram escritas com `def`. Não foi atalho:
com SQLAlchemy comum e o resto da pilha síncrona, `def` é a escolha que não
trava o processo. `async def` entra quando o que está do outro lado também
espera com `await`.
