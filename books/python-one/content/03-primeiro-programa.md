...
part: p1
kicker: "Dois Pythons na mesma manhã, e só um deles pode fechar o galpão."
goal: >-
...
:::story Funciona no galpão
A Bia sentou na cadeira que o Cacá deixou em 2021. No canto do monitor do
`GALPAO-02`, um post-it amarelo, com caneta de outra cor: *não desligar*.

Ela abriu o terminal.
...
No notebook dela, a mesma linha respondia 3.12.

— Aqui roda — disse a Dona Neuza, sem tirar o olho da balança. — Na sua
máquina, semana passada, não rodou. Apareceu um monte de linha vermelha e o
Cacá não estava.

O Elias, no telefone, de dentro do carro:

— Não mexe na 3.6. O fechamento da manhã depende dela. Instala a sua do
lado, e antes de rodar qualquer coisa descobre qual das duas você está
chamando.
:::

Rodar Python parece a parte mais simples do trabalho, e é onde se perde a
manhã: o `python` que responde no terminal quase nunca é o único da máquina.

O menor programa útil tem uma linha:
...
Pronto. O computador leu um arquivo e respondeu. Cada peça dessa linha tem
uma decisão dentro.

**`print`** escreve. É uma função, e por isso os parênteses são obrigatórios.
No Python 2, o do galpão, dava para escrever `print "oi"` sem eles. Esse
arquivo não roda mais numa versão atual — e é exatamente o tipo de surpresa
que a Bia encontrou na semana passada.

**`"Olá, Sabiá"`** é texto. As aspas marcam onde ele começa e onde termina.
Sem elas, `Olá` seria um nome, e o Python iria procurá-lo.

**`ola.py`** é só o nome do arquivo. O `.py` não tem poder especial: ele
avisa você, e avisa o editor, de que ali dentro tem Python.
...
descartável. Pode apagar sem medo; na próxima execução ela volta.
:::
...
vai falhar. Leia a mensagem inteira antes de continuar: ela diz o tipo que
você tentou somar e o tipo que o `+` aceitava.
:::
...
Os formatos que resolvem o dia a dia:
...
rodar e parou na hora exata em que a operação impossível apareceu. Esse é o
caráter da linguagem. O erro não é de forma — o arquivo é Python válido. O
erro é de conta, e a conta só é julgada quando chega a vez dela.
...
O Python só descobre que um nome não existe **no momento em que tenta
usá-lo**. Uma linguagem que confere tudo antes de rodar recusaria o arquivo
inteiro; o Python roda até ali e para na linha da conta. `precoo` não é um
erro de digitação que o arquivo anuncia na primeira linha. É um nome, e
nome só é julgado quando alguém pede o valor.
:::
