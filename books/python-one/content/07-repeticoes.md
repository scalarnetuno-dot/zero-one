...
:::story O Seu Onofre não saiu na lista
O fechamento da manhã listou cento e noventa e nove produtores. A cooperativa
tem duzentos.

— Faltou o Onofre — disse a Dona Neuza. — De novo. É sempre ele, e é sempre
segunda.

A Bia achou o laço no notebook. A lista tinha duzentos nomes. O laço pedia
os índices de zero até o tamanho da lista **menos um**, e em algum mês de
2022 alguém tinha somado outro menos um, "porque o último vinha em branco".

O último não vinha em branco. O último era o Seu Onofre, que está no fim
porque alguém ordenou por nome e não avisou.

— Tira o índice — disse o Elias. — Se você não precisa da posição, a posição
é só um lugar para errar a posição.
:::

Quase toda outra linguagem ensina o laço assim: crie um contador, teste o
contador, incremente o contador, use o contador como índice. O notebook do
Cacá foi escrito desse jeito. O Python deixa escrever diferente, e a
diferença é o Seu Onofre voltar para a lista.
...
Tabela: Início incluído, fim excluído, passo opcional. `range(0, 200)` tem
duzentos números, de 0 a 199. O 200 não entra — e era exatamente esse 200
que o notebook usava para decidir até onde ir, com um desconto a mais.
...
dia em que alguém apagar um item de uma e esquecer a outra, o `zip` para na
mais curta, sem reclamar, e o açafrão fica com o preço da beterraba.

Se o nome e o preço precisam viver juntos, eles precisam morar no mesmo
item. Duas listas são duas chances de desalinharem. Uma lista de pares não
deixa: apagou o par, apagou os dois.

```python
catalogo = [("Tomate", 8.90), ("Alface", 3.50)]
...
preço vezes 1,12, para cada preço em precos"*. A forma vale para lista. Para
dicionário e conjunto, a pontuação muda e a leitura é a mesma.
