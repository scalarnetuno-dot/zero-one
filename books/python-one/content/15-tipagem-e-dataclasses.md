...
:::story Oito reais, entre aspas
A planilha mandou o preço como texto. A célula estava formatada de um jeito,
o arquivo saiu de outro, e o que chegou na função foi `"8,90"`.

```text
>>> com_margem("8,90")
```

Não quebrou na entrada. Quebrou no meio, numa multiplicação, com uma
mensagem sobre `str` e `float` que não mencionava a planilha.

O Sérgio tinha pedido "colocar tipo", porque a palestra dizia que tipo
impede bug. A Bia colocou `preco: float` na assinatura e rodou de novo.

A mesma mensagem. A anotação estava lá. O Python tinha passado por ela sem
ler.

— Ela não trava — disse a Bia. — Ela avisa quem quiser olhar. Quem roda,
não olha.
:::

Anotação de tipo não é conferência. O interpretador passa por `preco: float`
e segue. Quem age em cima dela é outra ferramenta: o editor, um verificador,
ou uma biblioteca que decidiu ler a assinatura antes de chamar a função.

Vale separar as duas coisas, porque misturá-las é o que faz alguém achar
que `"8,90"` foi recusado quando não foi.
...
ser também um texto comum. O membro vira JSON sem conversão e entra numa
consulta sem tradução. `"ativo"` e `StatusProduto.ATIVO` passam a ser o
mesmo valor nos lugares em que o programa fala com o mundo de fora.
...
precisa de `dataclasses.asdict()` ou de outro modelo que saiba se
apresentar. O atrito é o preço de ter um tipo com nome: alguém precisa dizer
como ele vira texto. O dicionário nunca precisou, porque já era texto à
espera de uma chave.
:::
