...
Esse caminho vazio é o defeito mais fácil de não ver. Alguém pede um produto
que não existe, o `if` não entra, o programa segue, e a resposta sai como se
tivesse dado certo — com o corpo vazio. Quem pediu não recebe "não achei".
Recebe silêncio, e silêncio aqui parece sucesso.
...
:::pitfall
Uma escada com mais de quatro degraus é sinal de que falta um nome. As
trinta e sete combinações da margem não são trinta e sete decisões: são
meia dúzia de fatos sobre o produtor, combinados. Quando você se pegar
escrevendo o sexto `elif`, pare e pergunte que fato ainda não tem nome.
:::
...
A Bia foi ao quadro e desenhou a tabela de todas as combinações. Eram trinta
e sete.

O Elias olhou o quadro, olhou o relógio, e não disse nada.

— O conselho responde terça — disse a Helena, da porta. — E a sprint é esta.
:::
...
Uma função que trata pedido de fora começa com três ou quatro dessas saídas
e termina com duas linhas de trabalho de verdade. O caso ruim sai primeiro.
O caso feliz fica encostado na margem esquerda, onde ainda dá para ler.
...
A pergunta que a cooperativa faz o dia inteiro cabe numa linha: *isso
existe?* Estoque não informado, produto que saiu do catálogo, produtor sem
certificado. A forma da decisão é tratar o "não" antes do "sim":

```python title="ausencia.py" numbered
preco = None
...
A soma só roda quando há preço. Sem essa saída antecipada, `None + 1`
estoura, e a mensagem fala de `NoneType` — não de produto faltando. O
defeito real fica escondido atrás de um defeito secundário.
...
:::milestone
O programa agora decide. Recusar um preço negativo é este mesmo `if`, com a
condição virada para o caso ruim e a saída acontecendo antes da conta.
:::
