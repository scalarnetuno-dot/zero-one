...
Em Java, você anuncia o tipo antes de usar a variável e o compilador cobra a
promessa. Em Python, você não anuncia nada — e mesmo assim os tipos existem,
são rígidos e param o programa quando você os mistura.
...
## Os cinco tipos que sustentam uma API
...
como se fossem a mesma coisa está mentindo. No capítulo
@cap:modelando-com-pydantic isso vira uma decisão explícita de esquema:
campo obrigatório, campo opcional ou campo com padrão — três coisas que
iniciantes tratam como uma só.
...
No projeto deste livro, todo preço é `Decimal`. O banco vai guardá-lo como
`NUMERIC`, e o Pydantic vai convertê-lo no capítulo @cap:schemas. Use
`float` para peso, temperatura, percentual e média. Para dinheiro, nunca.
...
`ValueError` é o erro de "tipo certo, valor impossível" — e é exatamente o
que uma API precisa transformar em resposta `422` no capítulo
@cap:validacao, em vez de deixar virar `500`.
...
— A diferença é de três reais e quarenta — disse Dona Neuza, com o extrato
na mão. — Em doze mil.

Bia conferiu a soma três vezes. O código estava certo. As entradas estavam
certas. A saída estava errada.

Foi Elias quem olhou por cima do ombro, de passagem, e falou sem parar de
andar:
...
Bia trocou quatro linhas. A diferença virou zero.

— Isso está escrito em algum lugar? — ela perguntou.

— Está em todo lugar — disse Elias. — Só não está no tutorial.
:::
