...
Somar é `+`, subtrair é `-`, e isso não merece um capítulo. O que merece são
os cantos: a divisão que devolve um tipo diferente do que entrou, o `and`
que não devolve booleano, e a comparação encadeada que só o Python tem.
...
O primeiro resultado surpreende quem vem do Java ou do C: `7 / 2` em Python
dá `3.5`, não `3`. A divisão com uma barra **sempre** devolve `float`,
mesmo quando a conta é exata:
...
Tabela: O resto é o operador que responde "sobrou?" — e em código de
paginação, no capítulo @cap:paginacao, ele responde "tem mais uma página?".
