...
Quem digita `help(com_margem)` no REPL lê esse texto. Ele não desatualiza
sozinho — continua sendo você quem escreve —, mas pelo menos mora colado na
função, e não num arquivo que ninguém abre.

## Anotar o tipo, sem ilusão

Dá para escrever o tipo do parâmetro e o do retorno:
...
O Python ignora essas anotações na hora de rodar. `com_margem("oito")`
entra na função do mesmo jeito. Elas não convertem e não recusam. Quem lê é
o editor, e um verificador se você rodar um. A anotação é um recado para
quem lê o código, inclusive você em abril. Recado não é trava.
...
- Atribuição dentro da função cria nome local; precisar de `global` é um
  sintoma.
- Docstring é o texto que `help` mostra. Ela mora dentro da função.
:::
...
`return`, continuaria devolvendo `None` — porque `upper` devolve um texto
novo e não mexe no que já existia. Sem `return`, esse texto novo é criado
e jogado fora.
