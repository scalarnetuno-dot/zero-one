...
O `await` nessa função existe porque `call_next` não devolve a resposta na
hora. Devolve uma corrotina: a rota ainda vai trabalhar, e o resultado só
existe quando ela termina. Sem o `await`, o cabeçalho seria escrito em cima
desse objeto, não em cima da resposta, e o tempo medido seria o tempo de
criar a corrotina — quase zero.
...
4. **Deixe o `main.py` com o que é dele**: criar o app, ler configuração,
   incluir routers, middleware. Se sobrar regra de negócio lá, ela está no
   lugar errado: margem, estoque e permissão não moram no arquivo que sobe
   o servidor.
