> **Este livro é o volume 1 de PHP One** — capítulos 00 a 29, Partes 1 a 5.
> O roteiro abaixo é o do livro inteiro, mantido igual nos dois volumes; a
> seção "Parte 5 do volume 1" existe só aqui. O volume 2 renumera os
> capítulos 24 a 51 do roteiro como 1 a 28, e as partes 5 a 11 como 1 a 7.
...
# Parte 5 do volume 1 — Antes da web

*Existe só no volume 1, e não entra na numeração do livro inteiro. Fecha a
linguagem e o projeto para que o capítulo 1 do volume 2 comece direto no
HTTP. Cada capítulo termina com uma seção "O que o volume 2 faz com isto".*

## 24. Escopo e referências
Cena: "Dom Casmurro duas vezes" — `foreach` com `&` deixado aberto duplica o
último livro. Cópia de escalares e arrays, copy-on-write, objeto é o mesmo
objeto, `&`, `clone`/`__clone`, `static`/`global` (preparo para processos
que vivem mais que uma requisição).

## 25. Funções anônimas e closures
Cena: "Três linhas acima" — callable em texto quebra com namespace.
Formas de callable, `nome(...)`, `\Closure`, `use`, `fn`, `static fn`,
fábricas de filtro. Prévia: rota com closure e coleções do Laravel.

## 26. Manipulação de arquivos
Cena: "Na linha três" — `file()` numa planilha de 200 mil linhas estoura
16 MB (medido: 28,5 MB contra 0,4 MB com gerador). `__DIR__`, `fopen`/
`fgets`, `fgetcsv`/`fputcsv`, gerador com `yield`, BOM e codificação,
gravação atômica (temporário + `rename`), `flock`. Prévia: upload e
`Storage`.

## 27. Datas e horários
Cena: "O defeito do domingo" — fecha o defeito da Vera do cap. 2; o teste
do conserto falha depois das 21h (contêiner em UTC). Próximo dia aberto,
dias de atraso (data × instante), `Relogio` injetado (PSR-20), `+1 month`
em 31/01, `createFromFormat('!d/m/Y')` + `getLastErrors()`. Prévia: Carbon
e o `now()` congelado nos testes.

## 28. Erros e debug
Cena: "Zero livros, código zero" — importação noturna com
`> /dev/null 2>&1`. Níveis de erro, `set_error_handler` →
`ErrorException`, `bootstrap.php`, `@`, pilha lida de baixo para cima,
`var_dump`/Xdebug, `Registro` com contexto (PSR-3). Prévia: tratador do
Laravel, `Log`, `dd()`.

## 29. Como organizar um projeto PHP
Cena: "Onze vezes a mesma senha". `bin/ config/ public/ src/ tests/ var/`,
`autoload.files`, `composer scripts`, `.env` fora do Git + `config/`,
`bootstrap.php` como raiz de composição, `Servicos` (contêiner de closures
escrito à mão). Tabela acervo → Laravel. Termina no caderno da Tainá:
"o que chega do navegador até o PHP?" — a cena do `$_POST` vazio que abre
o volume 2. Marco: fim da Parte 5 e fim do volume 1.
